...
async function list(params = {}) {
  ...
  if (params.title) { where.push('t.title = ?'); vals.push(params.title); }
  ...
  const [res] = await pool.query(
    `INSERT INTO trainings
      (employee_id, title, category, training_date, trainer, validity_months, validity_days,
       expiration_date, process_classification, remarks, worker_line_status, take,
    ...
    [
      ...
      data.process_classification || '', data.remarks || '',
      data.worker_line_status || 'Floating', data.take || 1, user.id,
    ]
  );
...
async function update({ id, data } = {}) {
  ...
  const next = {
    ...
    remarks: 'remarks' in data ? data.remarks : training.remarks,
  ...
  await pool.query(
    ...
    [
      ...
      next.expiration_date, next.process_classification, next.remarks, next.worker_line_status, next.take, id,
    ]
  );
